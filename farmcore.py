...
def _db_update_sync(table: str, where_field: str, where_value: Any, payload: Dict[str, Any]) -> Dict[str, Any]:
    def _fn():
        return supabase.table(table).update(payload).eq(where_field, where_value).select("*").execute()
    return _safe_execute_sync(_fn, single_row=True)
